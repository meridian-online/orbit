...
Agent-to-Hugh prose follows the BLUF / Decision Brief contract — see card 0026 (`.orbit/cards/0026-executive-communication.yaml`). The contract is inlined below; the canonical source is `.orbit/STYLE.md`.
...
4. **Detail** — listed under "Available on request" as a one-line index, not dumped.
5. **Confidence** — one line (high / medium / low + key assumption) when the recommendation depends on uncertain inputs.

### Recommendation discipline

Imperative, not hedge-stacked. *"Run X on Y"* beats *"It might be worth considering whether perhaps X"*. State the call. Name the assumption if uncertain — don't sand the recommendation into mush. One concrete action per response, not a menu.

### The seven anti-patterns (proscribed)

1. **Lede-burying** — answer arrives after exposition. Lead with it.
...
British English. Concise, direct, warm but not chatty. Same register across every agent that responds to Hugh — no apologetic sandwiches, no peppy enthusiasm, no clinical cold.
